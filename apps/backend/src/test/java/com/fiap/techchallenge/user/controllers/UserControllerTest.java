...
/**
 * The authenticated caller's own profile (/users/me), plus the worker-only /users/{id} lookup.
 * getById's role-gate itself (CUSTOMER excluded, WORKER admitted, terminated-worker loses it) is
 * already covered end to end by auth/AuthorizationTest; this class covers the self-service routes
 * and getById's 404 case, which aren't.
 */
...
class UserControllerTest {
    ...
    @Test
    void aCustomerCanFetchTheirOwnProfile() throws Exception {
    ...
    @Test
    void aWorkerCanFetchTheirOwnProfile() throws Exception {
        Fixture worker = registerWorker(WorkerRole.MECHANIC);

        mvc.perform(get("/users/me")
                        .header("Authorization", "Bearer " + worker.accessToken()))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(worker.id().toString()));
    }

    @Test
    void fetchingMeWithoutATokenIsUnauthorized() throws Exception {
        mvc.perform(get("/users/me"))
                .andExpect(status().isUnauthorized());
    }

    @Test
    void aCustomerCanUpdateTheirOwnProfileViaMe() throws Exception {
        Fixture customer = registerCustomer();
                        ...
                        .content("""
                                {
                                  "firstName": "Nova",
                                  "lastName": "Souza",
                                  "phoneNumbers": [{"type": "MOBILE", "phone": "11999991111", "isPrimary": true}]
                                }"""))
                .andExpect(status().isOk())
    ...
    @Test
    void updatingMeWithABlankFirstNameIsRejected() throws Exception {
        Fixture customer = registerCustomer();
                        ...
                        .content("""
                                {
                                  "firstName": "",
                                  "lastName": "Souza",
                                  "phoneNumbers": [{"type": "MOBILE", "phone": "11999992222", "isPrimary": true}]
                                }"""))
                .andExpect(status().isBadRequest());
    }

    @Test
    void aWorkerCanGetAnyUserById() throws Exception {
        Fixture worker = registerWorker(WorkerRole.ATTENDANT);
    ...
    @Test
    void gettingAnUnknownUserByIdReturnsNotFound() throws Exception {
        Fixture worker = registerWorker(WorkerRole.MANAGER);
